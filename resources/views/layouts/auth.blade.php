...
<head>
    ...
    <title>@yield('title', 'PRODUCTS Management System')</title>
    ...
    <nav class="main-header navbar navbar-expand navbar-white navbar-light" style="background-color: black">
        ...
        <ul class="navbar-nav">
            <a href="{{ url('/') }}" class="brand-link">
                <span class="brand-text font-weight-light" style="color: gray">Products Management System</span>
            </a>
        </ul>
        <!-- Right navbar links -->
        <ul class="navbar-nav ml-auto">
            @if (Route::has('login'))
                @auth
                    <li class="nav-item">
                        <a href="{{ url('/dashboard') }}" class="nav-link" style="color: #9c0000">Control board</a>
                    </li>
                @else
                    <li class="nav-item">
                        <a href="{{ route('login') }}" class="nav-link" style="color: gray">Log in</a>
                    </li>
                    @if (Route::has('register'))
                        <li class="nav-item">
                            <a href="{{ route('register') }}" class="nav-link" style="color:gray">Register</a>
                        </li>
                    @endif
                @endauth
            @endif
